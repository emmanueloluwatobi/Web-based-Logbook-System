...
const NAV_ITEMS = [
  ...
  { label: "My Students", href: "/supervisor", icon: Users },
];

export function SupervisorSidebar({
  supervisor = {
    name: "Dr. O. A. Babalola",
    email: "o.babalola@eksu.edu.ng",
  ...
  const isActive = (item: typeof NAV_ITEMS[0]) => {
    // When on /supervisor, default "Overview" to active
  ...
  return (
    <>
            ...
            <div className="pt-4 border-t border-white/15 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div className="text-xs truncate max-w-[140px]">
                  <p className="font-semibold text-on-primary truncate">{supervisor.name}</p>
                  <p className="text-[11px] text-on-primary-container truncate">
                    {supervisor.role || "Academic Supervisor"}
                  </p>
                </div>
              </div>
              <button
      ...
      <aside className="hidden md:flex w-64 lg:w-70 bg-primary text-on-primary h-screen sticky top-0 flex-col justify-between p-4 border-r border-white/10 shrink-0 z-30 overflow-y-auto">
        <div>
          {/* Brand Wordmark */}
          <Link href="/supervisor" className="flex items-center gap-3 group pb-4 border-b border-white/15">
        ...
        {/* Bottom Supervisor Profile & Logout */}
